...
exports.recordDisbursementOnChain = async (campaignId, amount, reportHash) => {
  try {
    console.log("🔗 Recording disbursement...");

    const tx = await contract.recordDisbursement(
      campaignId.toString(),
      Number(amount),
      reportHash
    );
    ...
    console.log("✅ Disbursement recorded:", receipt.hash);
